export { CommunityAccess as CommunityAccessPage } from './CommunityAccess'
export { Friends as FriendsPage } from './Friends'
export { GiveawayDetail as GiveawayDetailPage } from './GiveawayDetail'
export { Giveaways as GiveawaysPage } from './Giveaways'
export { Home as HomePage } from './Home'
export { LeaderboardPage } from './LeaderboardPage'
export { MinesPage } from './MinesPage'
export { TowerPage } from './TowerPage'
export { RollPage } from './RollPage'
export { RollRoute } from './RollRoute'
export { RollDesktopPage } from './RollDesktopPage'
export { NotFoundPage } from './NotFoundPage'
export { OperationsHistoryPage } from './OperationsHistoryPage'
export { OrdersPage } from './OrdersPage'
export { Profile as ProfilePage } from './Profile'
export { ReferralBattlePage } from './ReferralBattlePage'
export { Shop as ShopPage } from './Shop'
export { Tasks as TasksPage } from './Tasks'
