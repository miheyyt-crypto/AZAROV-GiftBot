const steps = [
  ...
  },
  {
    ...
    text: 'Вот это и есть активация. Пока Kick не привязан, друг числится приглашённым, но не активным — и монет ещё нет ни у кого.',
    tone: 'purple' as const,
  },
  {
    title: 'Вы оба получаете по 500 монет',
    text: 'Один раз за каждого друга, сразу после привязки. Тебе и ему.',
    tone: 'gold' as const,
  },
  {
    title: 'Каждые 5 друзей — реферальный кейс',
    text: 'Копится само по числу приглашённых: как только счётчик доходит до цели, кейс появляется в разделе «Кейсы».',
    tone: 'purple' as const,
  },
] as const
    ...
    <section className="rounded-[24px] border border-white/8 bg-[#16121f]/95 p-5">
      ...
      <div className="mt-5 rounded-[16px] border border-white/6 bg-[#0f0d16] px-4 py-3.5">
        <p className="text-sm leading-relaxed text-[#a1a1aa]">
          Один Kick — один аккаунт, навсегда.
          <br />
          Привязать один и тот же канал на два профиля нельзя, поэтому «пригласить
          самого себя» не сработает.
        </p>
      </div>
    </section>
  )
